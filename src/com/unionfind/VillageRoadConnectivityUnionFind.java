 ...
 */
/**
 * 问题：村村通道路连通问题，求还需要建设多少条路才能连通所有村庄。
 * 方法：使用并查集合并已有道路两端的村庄，最后统计连通分量数量。
 * 解题思路：初始化时每个村庄都是独立集合；读取每条已有道路后，将道路两端村庄所在集合合并。
 * 所有道路处理完后，统计集合代表元数量，若有 k 个连通分量，则还需要 k-1 条路将它们连接起来。
 * 时间复杂度：O((n+m)α(n))，n 为村庄数，m 为已有道路数，α 为反阿克曼函数。
 * 空间复杂度：O(n)，需要父节点数组维护并查集。
 */
