...
/**
 * 问题：0/1 背包问题。
 * 方法：使用动态规划二维表求最大价值，并通过路径表还原选择的物品。
 * 解题思路：V[i][j] 表示前 i 个物品在容量 j 下能取得的最大价值。
 * 如果当前物品重量超过容量，则不能选；否则在“不选当前物品”和“选当前物品”之间取最大值，并用 path 表记录选中情况以便还原方案。
 * 时间复杂度：O(nC)，n 为物品数量，C 为背包容量。
 * 空间复杂度：O(nC)，当前实现使用二维价值表和路径表。
 */
