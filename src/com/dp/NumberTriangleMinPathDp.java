...
/* 用动态规划问题去解决数塔问题
   输入：二维数组a[n][n]
   输出：数塔的最大数值和其所经过的路径
   1. 初始化数值minAdd的最后一行为数塔的底层数据：
      for(j=0;j<n;j++)
         minAdd[n-1][j]=a[n-1][j];
   2. 从第n-1层开始直到第一层对下三角元素minAdd[i][j]执行下述操作：
      2.1 minAdd[i][j]=a[i][j]+min{minAdd[i+1][j],minAdd[i+1][j+1]};
...
/**
 * 问题：数字三角形最小路径和。
 * 方法：使用动态规划自底向上填表，并记录每层路径选择。
 * 解题思路：从三角形底层向上计算每个位置到达底部的最小路径和。
 * 对位置 (i,j)，比较下一层 j 和 j+1 两个可达位置，选择较小值加上当前数字，并记录路径方向。
 * 时间复杂度：O(n^2)，n 为三角形层数。
 * 空间复杂度：O(n^2)，需要保存动态规划表和路径表。
 */
public class NumberTriangleMinPathDp {
    ...
    public static int countTower(int a[][], int n) {
        ...
        for (int i = 0; i < n; i++) {                  //首先对最后倒数第二层进行初始化，将其值赋值给minAdd这个用来保存最小值的数组
        ...
        System.out.println("");
        return minAdd[0][0];                        //返回最大数值和，即最终的决策结果
    }
}
