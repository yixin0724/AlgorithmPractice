...
/**
 * 问题：求两个序列的最长公共子序列。
 * 方法：使用动态规划二维表记录子问题长度，并通过状态矩阵回溯输出子序列。
 * 解题思路：L[i][j] 表示 X 的前 i 个字符与 Y 的前 j 个字符的最长公共子序列长度。
 * 当两个字符相等时从左上角转移并加一，不相等时取左侧和上侧较大值；状态矩阵 S 记录转移方向，用于最后回溯出具体序列。
 * 时间复杂度：O(nm)，n 和 m 分别为两个序列长度。
 * 空间复杂度：O(nm)，需要长度表和状态表。
 */
