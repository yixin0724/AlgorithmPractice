...
/**
 * 问题：打家劫舍，不能同时选择相邻房屋，求可偷取的最大金额。
 * 方法：使用动态规划，状态为偷到当前房屋前可获得的最大收益。
 * 解题思路：对第 i 间房屋有两种选择：偷它，则收益为 dp[i-2]+nums[i]；不偷它，则收益为 dp[i-1]。
 * 每一步取两种选择的最大值，最终 dp[n-1] 就是答案。
 * 时间复杂度：O(n)，n 为房屋数量。
 * 空间复杂度：O(n)，当前实现使用 dp 数组保存每个位置的最优值。
 */
